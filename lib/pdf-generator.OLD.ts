...
const CORES = {
  ...
  reciproca: {
    primaria: [34, 197, 94],      // green-500
    clara: [240, 253, 244],        // green-50
    escura: [22, 163, 74],         // green-600
  },
  templos: {
    primaria: [168, 85, 247],      // purple-500
    clara: [250, 245, 255],        // purple-50
    escura: [147, 51, 234],        // purple-600
  },
  partidos: {
    primaria: [249, 115, 22],      // orange-500
    clara: [255, 247, 237],        // orange-50
    escura: [234, 88, 12],         // orange-600
  },
  livros: {
    primaria: [99, 102, 241],      // indigo-500
    clara: [238, 242, 255],        // indigo-50
    escura: [79, 70, 229],         // indigo-600
  },
  fonogramas: {
    primaria: [236, 72, 153],      // pink-500
    clara: [253, 242, 248],        // pink-50
    escura: [219, 39, 119],        // pink-600
  },
};

// Cores institucionais
const COR_AZUL = [59, 130, 246];      // blue-500
const COR_AZUL_ESCURO = [30, 64, 175]; // blue-800
const COR_CINZA = [107, 114, 128];     // gray-500
const COR_CINZA_CLARO = [249, 250, 251]; // gray-50

// Informações sobre cada tipo
const tiposImunidade: Record<string, { titulo: string; emoji: string; fundamento: string }> = {
  reciproca: {
...
export const gerarPDFRequerimento = (dados: DadosFormulario) => {
  ...
  // Adicionar título de seção com ícone
  const addSectionTitle = (emoji: string, titulo: string, destaque: boolean = false) => {
    checkPageBreak(15);
    ...
    doc.text(`${emoji} ${titulo}`, margin, currentY + 5);
    currentY += 10;
  };
  ...
  // ========================================
  // TIPO DE IMUNIDADE (com emoji e fundamento)
  // ========================================
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...coresTipo.escura);
  doc.text(`${infoTipo.emoji} ${infoTipo.titulo}`, pageWidth / 2, currentY + 5, { align: 'center' });
  currentY += 8;
  ...
  addSectionTitle('⚖️', `${numeroSecaoDecl}. DECLARAÇÕES ESPECÍFICAS`, true);
  currentY += 5;
  ...
  doc.text('⚠️ DECLARAÇÃO DE RESPONSABILIDADE GERAL', pageWidth / 2, currentY, { align: 'center' });
  currentY += 8;
  ...
  const linhasDeclaracao = doc.splitTextToSize(declaracao, pageWidth - 2 * margin - 15);
  doc.text(linhasDeclaracao, margin, currentY);
  currentY += 35;
  ...
  const nomeArquivo = `Requerimento_Imunidade_${infoTipo.emoji}_${dados.razaoSocial.substring(0, 20).replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.pdf`;
